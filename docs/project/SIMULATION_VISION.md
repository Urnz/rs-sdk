...
Mielőtt új szükséglet-, idő-, aging-, durability- vagy hosszú production
rendszert vezetünk be, külön integrációs mérföldkő szükséges: a már elkészült
0–14. fázis MVP-jeiből működő, restart-álló autonóm proto-társadalmat kell
összeállítani. Ez bizonyítja, hogy a player agentek a jelenlegi mechanikákból
emberi mikromenedzsment nélkül képesek folyamatos életciklust alkotni.
...
Ezek működő technikai alapok, de a capability-audit szerint több közülük még
izolált vagy adminvezérelt MVP. Egy player agent eseményre már képes allowlistelt
verified skillt indítani és ugyanazon gateway-életciklusban skill-exit után
újratervezni, de nincs teljes idle/startup supervisor, tartós event-admission és
restart recovery. A Business/Faction agent portoknak sincs még folyamatos
döntési lifecycle-juk. A rendszer ezért még autonóm proto-társadalmat sem
bizonyított, és ettől különösen nem alkot önfenntartó, véges életű, teljes
gazdaságot és társadalmat.

## Köztes mérföldkő: autonóm proto-társadalom

Az első következő milestone a meglévő rendszerek integrációja egy kicsi,
reprodukálható, tartósan működő agentközösséggé. Körülbelül 5–10 exact
avatarhoz kötött player agent ugyanabban a világban:

- fresh contextből saját célt folytat vagy policy szerint következő célt választ;
- csak szükség esetén kér ritka, magas szintű LLM-döntést;
- ismert, verified és bounded agent skillt választ;
- a skillt a meglévő supervisor/executor útvonalon hajtja végre;
- journal, domain evidence, memory és goal progress alapján frissíti állapotát;
- skill-, goal-, offer-, contract-, Business- és releváns world eventre ébred;
- cooldown, queue-, költség- és failure limitek között újratervez;
- engine/gateway restart után tartós enrollmentből, célból, memóriából,
  assetből, ismert skillből és folyamatállapotból folytatja működését.

Ez nem tickenkénti LLM-loop. Az idle/due wake-up csak scheduler admission, a
modell továbbra is ritka stratégiai döntést hoz; a végrehajtás magas szintű
verified skill vagy szűk, típusos domain tool.

A milestone a meglévő domainek közötti integrációt érleli. A Business, contract,
Property, banking vagy Governance hiányzó agentbekötése nem új párhuzamos domain,
hanem a meglévő modul adaptere, eseménye, context-vetülete és lifecycle-ja.

### Autonóm proto-társadalom és önfenntartó társadalom

A két mérföldkő nem azonos.

1. **Autonóm proto-társadalom:** az agentek a jelenlegi RuneScape-mechanikák,
   verified skillek, NPC shopok és már elkészült gazdasági domainek segítségével
   maguktól működnek. A kezdőállapot adhat toolt, inputot, coint, Propertyt,
   Business-t és gazdasági lehetőséget. A vanilla NPC shop vagy más külső
   gazdasági forrás megmaradhat, ha explicit és mérhető.
2. **Önfenntartó társadalom:** egy későbbi milestone-ban a needs, housing,
   consumption, durability, production workflow, facility, lifecycle és
   népesség rendszerek belső keresletet és pótlási kényszert hoznak létre. A
   közösség ekkor képes alapvető szükségleteit indokolatlan külső injekció nélkül
   fenntartani.

Az első milestone azt bizonyítja, hogy az agent runtime és az MVP-domainek
összeállnak. A második azt vizsgálja, hogy a társadalom gazdaságilag képes-e
saját fennmaradását újratermelni. Az autonóm működés szükséges, de nem elégséges
feltétele az önfenntartásnak.

A proto-society acceptance, fixture, approval-határok és konkrét integrációs
hiányok az
[autonóm proto-társadalom auditjában](AUTONOMOUS_PROTO_SOCIETY_AUDIT.md) és az
[aktív 15. fázisban](TASKS.md) találhatók.
...
- enrolled/idle/planning/executing/backoff/quarantined/recovering agentállapot;
- event inbox/outbox, lease, next wake-up, utolsó hiteles progress és failure fingerprint;
...
1. a 0–14. fázis MVP-jeinek autonóm proto-társadalommá integrálása,
   restart-álló supervisorral, event lifecycle-lal, approval policyval és kis
   Varrock fixture-rel;
2. szimulációs idő és world genesis;
3. attribute-ok és skillpotential;
4. hunger/fatigue + alvás;
5. meglévő épületekre épülő housing/tenancy;
6. food consumption és egyetlen tool-lifecycle vertikális szelet;
7. gazdaságilag jelentős NPC szolgáltatók agentizálása és Business 2.0;
8. adatvezérelt production workflow, facility, storage és logisztika;
9. első valódi tőkeeszköz/facility-upgrade;
10. tulajdonvédelem, Thieving-bővítés és justice MVP;
11. health, öregedés, halál, estate és népesség-utánpótlás;
12. hosszabb önfenntartási és társadalmi kontrollkísérletek.
...
- [Autonóm proto-társadalom audit](AUTONOMOUS_PROTO_SOCIETY_AUDIT.md)
