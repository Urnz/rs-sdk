...
import { buildExperimentRespawnWorldModEntry, EXPERIMENT_RESPAWN_MOD_ID,
    verifyExperimentRespawnWorldMod } from './experiment-respawn-adapter.js';
import { buildExperimentMarketWorldModEntry, EXPERIMENT_MARKET_MOD_ID,
    verifyExperimentMarketWorldMod } from './experiment-market-adapter.js';
import { buildExperimentFinishedProductWorldModEntry, EXPERIMENT_FINISHED_PRODUCT_MOD_ID,
...
export async function handleAdminRequest(req: Request, url: URL, context: AdminRouteContext): Promise<Response | null> {
    ...
    try {
        ...
        const experimentRespawnApplyMatch = url.pathname.match(/^\/api\/admin\/experiment-parameter-profiles\/([^/]+)\/([^/]+)\/apply-respawn$/);
        if (req.method === 'POST' && experimentRespawnApplyMatch?.[1] && experimentRespawnApplyMatch[2]) {
            const profileId = decodeURIComponent(experimentRespawnApplyMatch[1]);
            const version = decodeURIComponent(experimentRespawnApplyMatch[2]);
            const body = await requestBody(req);
            const reason = text(body, 'reason', true);
            const store = new ExperimentParameterStore(experimentParametersDbPath);
            try {
                const profile = store.get(profileId, version);
                if (!profile) throw new Error('A kiválasztott paraméterprofil nem található.');
                const entry = buildExperimentRespawnWorldModEntry(profile);
                const beforeView = await listWorldMods();
                const update = await updateWorldMod(EXPERIMENT_RESPAWN_MOD_ID, entry, beforeView.revision,
                    undefined, undefined, undefined, reason);
                const activation = await requestWorldModHotReload();
                const activeView = await listWorldMods();
                const activeMod = verifyExperimentRespawnWorldMod(profile, activeView.mods);
                await appendAudit({ operator: 'local-admin', action: 'experiment-parameter-profile.apply-respawn', reason,
                    success: true, before: { revision: update.before.revision, mod: update.before.mods[EXPERIMENT_RESPAWN_MOD_ID] },
                    after: { revision: update.after.revision, mod: activeMod.active, profileId, version,
                        profileDigest: profile.digest, activation, backupId: update.backup.id } });
                return json({ ok: true, profileId, version, profileDigest: profile.digest,
                    revision: update.after.revision, activeRevision: activeView.activeRevision,
        ...
        const experimentMarketApplyMatch = url.pathname.match(/^\/api\/admin\/experiment-parameter-profiles\/([^/]+)\/([^/]+)\/apply-market$/);
        if (req.method === 'POST' && experimentMarketApplyMatch?.[1] && experimentMarketApplyMatch[2]) {
            const profileId = decodeURIComponent(experimentMarketApplyMatch[1]);
            const version = decodeURIComponent(experimentMarketApplyMatch[2]);
            const body = await requestBody(req);
            const reason = text(body, 'reason', true);
            const store = new ExperimentParameterStore(experimentParametersDbPath);
            try {
                const profile = store.get(profileId, version);
                if (!profile) throw new Error('A kiválasztott paraméterprofil nem található.');
                const entry = buildExperimentMarketWorldModEntry(profile);
                const beforeView = await listWorldMods();
                const update = await updateWorldMod(EXPERIMENT_MARKET_MOD_ID, entry, beforeView.revision,
                    undefined, undefined, undefined, reason);
                const activation = await requestWorldModHotReload();
                const activeView = await listWorldMods();
                const activeMod = verifyExperimentMarketWorldMod(profile, activeView.mods);
                await appendAudit({ operator: 'local-admin', action: 'experiment-parameter-profile.apply-market', reason,
                    success: true, before: { revision: update.before.revision, mod: update.before.mods[EXPERIMENT_MARKET_MOD_ID] },
                    after: { revision: update.after.revision, mod: activeMod.active, profileId, version,
                        profileDigest: profile.digest, activation, backupId: update.backup.id } });
                return json({ ok: true, profileId, version, profileDigest: profile.digest,
                    revision: update.after.revision, activeRevision: activeView.activeRevision,
        ...
        const experimentFinishedProductApplyMatch = url.pathname.match(/^\/api\/admin\/experiment-parameter-profiles\/([^/]+)\/([^/]+)\/apply-finished-products$/);
        if (req.method === 'POST' && experimentFinishedProductApplyMatch?.[1] && experimentFinishedProductApplyMatch[2]) {
            const profileId = decodeURIComponent(experimentFinishedProductApplyMatch[1]);
            const version = decodeURIComponent(experimentFinishedProductApplyMatch[2]);
            const body = await requestBody(req);
            const reason = text(body, 'reason', true);
            const store = new ExperimentParameterStore(experimentParametersDbPath);
            try {
                const profile = store.get(profileId, version);
                if (!profile) throw new Error('A kiválasztott paraméterprofil nem található.');
                const entry = buildExperimentFinishedProductWorldModEntry(profile);
                const beforeView = await listWorldMods();
                const update = await updateWorldMod(EXPERIMENT_FINISHED_PRODUCT_MOD_ID, entry, beforeView.revision,
                    undefined, undefined, undefined, reason);
                const activation = await requestWorldModHotReload();
                const activeView = await listWorldMods();
                const activeMod = verifyExperimentFinishedProductWorldMod(profile, activeView.mods);
                await appendAudit({ operator: 'local-admin', action: 'experiment-parameter-profile.apply-finished-products',
                    reason, success: true,
                    before: { revision: update.before.revision,
                        mod: update.before.mods[EXPERIMENT_FINISHED_PRODUCT_MOD_ID] },
                    after: { revision: update.after.revision, mod: activeMod.active, profileId, version,
                        profileDigest: profile.digest, activation, backupId: update.backup.id } });
                return json({ ok: true, profileId, version, profileDigest: profile.digest,
                    revision: update.after.revision, activeRevision: activeView.activeRevision,
                ...
                await appendAudit({ operator: 'local-admin',
                    action: 'experiment-parameter-profile.apply-finished-products', reason,
                    success: false, after: { profileId, version }, error: String(error) });
                throw error;
            } finally { store.close(); }
        }
